class Game {
  ...
  private setSolid() {
    for (let y = 0; y < this._currentSchema.length; y++) {
      for (let x = 0; x < this._currentSchema[y].length; x++) {
        if (this._currentSchema[y][x] === 1) {
          this._landed[y + this._currentY - 1][
            x + this._currentX
  ...
  private onPressKeyboard(event) {
    switch (event.code) {
      case 'ArrowUP':
        ...
        if (
          !this.checkCollision(newSchema, 0, 0) &&
  ...
  private static rotateClockwise(
  ...
  private checkLines() {
    ...
    for (const line of linesToShift) this.shiftLines(line);
  }
